...
export default function VideoPlayer({videoUrl, thumbnail, title, onVideoCompleted}: VideoPlayerProps) {
  ...
  return <View ref={rootView} style={styles.container}>
    ...
    {Platform.OS === 'web' ? React.createElement('video', {ref: webVideo, src: videoUrl, autoPlay: true, muted: true, loop: true, playsInline: true, onLoadedMetadata: (e: any) => setDuration(e.currentTarget.duration || 0), onTimeUpdate: (e: any) => {setCurrent(e.currentTarget.currentTime || 0); setDuration(e.currentTarget.duration || 0);}, onPlay: () => setPlaying(true), onPause: () => setPlaying(false), onEnded: () => {setDone(false); onVideoCompleted?.();}, style: {width: '100%', height: '100%', objectFit: 'contain', backgroundColor: '#000', borderRadius: 18}}) : <VideoView player={player} style={styles.video} contentFit="contain" nativeControls={false} />}
...
const styles = StyleSheet.create({container:{aspectRatio:16/9,width:'100%',marginTop:24,marginHorizontal:0,backgroundColor:'#000',borderTopLeftRadius:18,borderTopRightRadius:18,borderBottomLeftRadius:0,borderBottomRightRadius:0,overflow:'hidden',borderWidth:1,borderColor:'#26302d',shadowColor:'#000',shadowOpacity:.22,shadowRadius:10,shadowOffset:{width:0,height:5},elevation:5},video:{...StyleSheet.absoluteFillObject,borderTopLeftRadius:18,borderTopRightRadius:18,borderBottomLeftRadius:0,borderBottomRightRadius:0},thumbnail:{...StyleSheet.absoluteFillObject,width:'100%',height:'100%',borderTopLeftRadius:18,borderTopRightRadius:18},overlay:{backgroundColor:'transparent'},center:{flex:1,alignItems:'center',justifyContent:'center'},play:{color:'#fff',fontSize:34,textShadow:'0px 2px 8px #000'},bottom:{position:'absolute',bottom:0,left:0,right:0,paddingHorizontal:10,paddingVertical:6,backgroundColor:'rgba(0,0,0,.62)'},time:{color:'#fff',fontSize:11},slider:{height:24,padding:0},row:{flexDirection:'row',alignItems:'center',justifyContent:'space-between'},control:{color:'#fff',fontSize:21},speeds:{flexDirection:'row',gap:7},speed:{color:'#ddd',fontSize:11},active:{color:'#16df92',fontWeight:'700'},error:{color:'#fff',position:'absolute',alignSelf:'center',top:'45%',padding:10}}
);
